...
The homepage (`/`) carries a second visual layer on top of Night Trail: a single-viewport hero built from a concrete external reference (a liquid-metal, high-craft AI-SaaS landing page) the team pointed to directly, the same way the reference for Night Trail itself was chosen. This is scoped to `/` only — `/catalog`, `/chat`, `/dashboard`, and `/profile` are unaffected, since they're Operate surfaces (task screens), not a Persuade surface (the landing page). Originally this layer also ran its own color palette (pure black, cool-blue-tinted glows, a white chrome primary button); that deviation was retired so the homepage reads as the same product as the rest of the app on first glance, not a different one bolted on.

**What's shared with Night Trail now:**
- Colors: the hero's local tokens (`--bg`, `--muted`, `--border`, plus every button and glow) resolve to the same root custom properties as the rest of the app (`--ground`, `--text`, `--text-muted`, `--surface`, `--border-strong`, `--blaze`) — set once in `globals.css` and inherited into the CSS Module rather than redeclared. The primary CTA is Blaze-filled like every other primary button in the product; the ghost/secondary button and nav pills use neutral white-based glass rather than the old cool-blue-tinted hover glow, since Blaze is reserved for the primary action per the One-Glow Rule.

**What's still different here, and why it doesn't leak:**
- Inter + Instrument Serif (italic accent word only) rather than Zilla Slab/Archivo, liquid-metal gradient nav pills and glass buttons rather than Night Trail's flat pill/border components — implemented as a CSS Module (`page.module.css`) plus two small client components (`HeaderNav.tsx`, `AppearFallback.tsx`), never in `globals.css`, so none of it can bleed into the app screens.
...
**Named Rule — The One-Surface Exception (type and shape only):** the landing hero is the only page in the system allowed to deviate from Night Trail's type family and component shapes (Inter/Instrument Serif, glass buttons, liquid-metal nav pills). It no longer deviates on color — don't reintroduce a separate color palette here, and don't extend the type/shape exception past `/`.
...
- **Don't** extend the landing hero's Inter/Instrument-Serif type or liquid-metal component shapes past `/` (The One-Surface Exception) — and don't invent adoption/traction stats there; every stat must be a true product fact.
- **Don't** give the landing hero its own color palette again — its tokens must resolve to the same root custom properties (`--ground`, `--text`, `--blaze`, etc.) as the rest of the app, not a separate pure-black/cool-tinted set.
